...
function ensureOrderSheetSchema(sheet) {
  ...
  // 1) Original SHOPLIVE structure: title row 1 + headers row 2, 16 columns.
  if (row2[0] === 'Statut' && row2[1] === 'RequestID') {
    sheet.insertColumnBefore(1);                 // StoreKey
    sheet.getRange(HEADER_ROW, 1).setValue('StoreKey');
    sheet.insertColumnAfter(sheet.getLastColumn()); // OrderData
    sheet.getRange(HEADER_ROW, SHEET_WIDTH).setValue('OrderData');
  }
  // 2) Previous Multi-Tenant structure: headers on row 1.
